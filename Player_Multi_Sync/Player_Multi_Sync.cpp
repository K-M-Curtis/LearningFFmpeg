...
double get_audio_clock(MediaState* media_state) {
  ...
  // 计算当前音频解码数据缓存索引位置，音频缓冲区还没有播放的数据
...
void video_refresh_timer(void* userdata) {
  ...
  if (video_state->video_stream) {
    ...
    if (video_state->picture_queue_size == 0) {
      ...
      // save for next time, 保存延时和pts，等待下次使用
      // 保存上一帧图像的动态刷新延迟时间
...
int main(int argc, char* argv[]) {
  ...
  while (media_state->quit != 1) {
    /*这里使用WaitEvent会有问题，会一直在等第一帧，画面是黑频，
    也有查过资料，怀疑是WaitEvent有点问题
    -- 该问题前面有说明过，调试发现是因为这个与SDL的window线程不在一个线程
    上所导致的问题，已经处理了，把window的创建放到主线程*/
    SDL_WaitEvent(&event);
