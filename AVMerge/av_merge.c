// 这里时音视频文件合成的简单例子，使用的是提取的音频文件和视频文件进行测试
// 例子中使用 .h264视频文件和 .aac音频文件合成一个 .mp4文件的操作，并使用ffplay进行播放
#include <stdio.h>
...
int main(int argc, char* argv[]) {
  ...
  if ((err_code = avformat_find_stream_info(ifmt_ctx1, 0)) < 0) {
    av_strerror(err_code, errors, ERROR_STR_SIZE);
    av_log(NULL, AV_LOG_ERROR, "Failed to retrieve input stream info, %s, %d(%s) \n",
      src_file1, err_code, errors);
    goto __FAIL;
  }
  ...
  if ((err_code = avformat_open_input(&ifmt_ctx2, src_file2, 0, 0)) < 0) {
    av_strerror(err_code, errors, ERROR_STR_SIZE);
    av_log(NULL, AV_LOG_ERROR, "Could not open the second src file, %s, %d(%s)\n",
