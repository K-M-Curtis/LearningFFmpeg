...
typedef struct MediaState {
  ...
  // 音频不同步计数（音频时钟与主同步源存在不同步的次数）
  int audio_diff_avg_count;

  // video/audio_clock save pts of last decoded frame / predicted pts of next decoded frame
  // keep track of how much time has passed according to the video
  // 视频已播放时间，用于预测下一帧动态刷新的延迟时间
  double video_clock;
...
// data成员被标记为"FLUSH"的AVPacket类型对象，
// 解码操作时在数据包队列中遇到某个包的data成员为"FLUSH"时，执行重置解码器操作
// 在执行[快进]/[快退]操作后，ffmpeg需要执行重置解码器操作
...
static void Packet_Queue_Flush(PacketQueue* packet_queue) {
  AVPacketList *pkt_list, *pkt_tmp_list;
  SDL_LockMutex(packet_queue->mutex);
  // 遍历队列所有节点
  for (pkt_list = packet_queue->first_pkt; pkt_list != NULL;
       pkt_list = pkt_tmp_list) {
    pkt_tmp_list = pkt_list->next;
    // 当前节点引用计数-1
    if (pkt_list) av_packet_unref(&pkt_list->pkt);
    // 释放当前节点缓存
    av_freep(pkt_list);
  }
  packet_queue->last_pkt = NULL;
  packet_queue->first_pkt = NULL;
...
/*-----------取得视频时钟-----------
* 即取得当前播放视频帧的pts，以视频时钟pts作为音视频同步基准，
* return the current time offset of the video currently being played
* 该值为当前帧时间戳pts+一个微小的修正值delta
* 在毫秒级别上，若取得视频时钟(即当前帧pts)的时刻，与调用视频时钟的时刻(如将音频同步到该视频pts时刻)存在延迟
...
* 本例子（AV_SYNC_VIDEO_MASTER）仅为展示同步到视频时钟的方法，一般情况下同步到视频时钟仅作为辅助的同步方案
--------------------------------*/
double get_video_clock(MediaState* media_state) {
  double delta =
      (av_gettime() - media_state->video_current_pts_time) / 1000000.0;
  return media_state->video_current_pts + delta;
}

// 取得系统时间，以系统时钟作为同步基准
double get_external_clock(MediaState* media_state) {
  // 取得系统当前时间，以1/1000000秒为单位，便于在各个平台移植
  return av_gettime() / 1000000.0;
}

// 取得主时钟(基准时钟)
double get_master_clock(MediaState* media_state) {
  if (media_state->av_sync_type == AV_SYNC_VIDEO_MASTER) {
    // 返回视频时钟
    return get_video_clock(media_state);
  } else if (media_state->av_sync_type == AV_SYNC_AUDIO_MASTER) {
    // 返回音频时钟
    return get_audio_clock(media_state);
  } else {
    // 返回系统时钟
    return get_external_clock(media_state);
  }
}
...
* 该函数比对音频时钟与主同步源的时差，通过动态丢帧(或插值)部分音频数据，以起到
* 减少(或增加)音频播放时长，减少与主同步源时差的作用
* 该函数对音频缓存数据进行丢帧(或插值)，返回丢帧(或插值)后的音频数据长度
* 因为音频同步可能带来输出声音不连续等副作用，该函数通过音频不同步次数(audio_diff_avg_count)及时差
* 均值(avg_diff)来约束音频的同步过程
---------------------------*/
// Add or subtract samples to get a better sync, return new audio buffer size
int synchronize_audio(MediaState* media_state, short* samples, int samples_size,
                      double pts) {
  double ref_clock;  // 主同步源(基准时钟)
  // 每组音频数据字节数=每声道数据字节数 * 声道数
  int pcm_bytes = 2 * media_state->audio_codec_ctx->ch_layout.nb_channels;

  /* if audio not master, then we try to remove or add samples to correct the clock */
  // 检查主同步源，若同步源不是音频时钟的情况下，执行以下代码
  if (media_state->av_sync_type != AV_SYNC_AUDIO_MASTER) {
    double diff;             // 音频帧播放间与主同步源时差
    double avg_diff;         // 采样不同步平均值
    int wanted_size;         // 经过丢帧(或插值)后的缓存长度
    int min_size, max_size;  // 缓存长度最大/最小值

    // 取得当前主同步源，以主同步源为基准时间
    ref_clock = get_master_clock(media_state);
    // 计算音频时钟与当前主同步源的时差
    diff = get_audio_clock(media_state) - ref_clock;

    // 检查音频是否处于不同步状态(通过AV_NOSYNC_THRESHOLD限制丢弃的音频数据长度，避免出现声音不连续)
    if (diff < AV_NOSYNC_THRESHOLD) {
      // 对时差加权累加(离当前播放时间近的时差权值系数大)
      media_state->audio_diff_cum =
          diff + media_state->audio_diff_avg_coef * media_state->audio_diff_cum;

      // 将音频不同步计数与阈值进行比对
      if (media_state->audio_diff_avg_count < AUDIO_DIFF_AVG_NB) {
        // not enough measures to have a correct estimate
        // 音频不同步计数更新
        media_state->audio_diff_avg_count++;
      } else {
        // 当音频不同步次数超过阈值限定后，触发音频同步操作
        // 计算时差均值(等比级数几何平均数)
        avg_diff = media_state->audio_diff_cum *
                   (1.0 - media_state->audio_diff_avg_coef);

        // 比对时差均值与时差阈值
        if (fabs(avg_diff) >= media_state->audio_diff_threshold) {
          // 根据时差换算同步后的缓存长度
          wanted_size =
              samples_size +
              ((int)(diff * media_state->audio_codec_ctx->sample_rate) *
               pcm_bytes);
          // 同步后的缓存长度最小值
          min_size =
              samples_size * ((100 - SAMPLE_CORRECTION_PERCENT_MAX) / 100);
          // 同步后的缓存长度最大值
          max_size =
              samples_size * ((100 + SAMPLE_CORRECTION_PERCENT_MAX) / 100);
          ...
          // 比对同步后的音频缓存数据长度与原始缓存长度
          if (wanted_size < samples_size) {
            // remove samples，丢帧方式，用丢帧后的音频缓存长度更新原始缓存长度
            samples_size = wanted_size;
          } else if (wanted_size > samples_size) {
            // 若同步后缓存长度大于当前缓存长度
            // add samples by copying final sample，补帧的方式，通过复制最后一个音频数据进行插值
            // 计算缓存长度与原始缓存长度间的差值(需要插值的音频数据组数)
            int nb = wanted_size - samples_size;
            // 取得缓存末端数据指针
            uint8_t* samples_end = (uint8_t*)samples + samples_size - pcm_bytes;
            // 初始插值位置|<-----samples----->||q|
            uint8_t* q = samples_end + pcm_bytes;
            // 检查插值音频组数(每组包括两个声道的pcm数据)
            while (nb > 0) {
      ...
      media_state->audio_diff_avg_count = 0;  // 音频不同步计数重置
      media_state->audio_diff_cum = 0;        // 音频累计时差重置
    }
  }
  // 返回发送到声卡的音频缓存字节数，音频同步后的缓存长度
  return samples_size;
}
    ...
    if (audio_state->audio_buf_index >= audio_state->audio_buf_size) {
      ...
      if (audio_size < 0) {
        ...
        audio_size = synchronize_audio(
            audio_state, (int16_t*)audio_state->audio_buf, audio_size, pts);
...
void video_refresh_timer(void* userdata) {
  ...
  if (video_state->video_stream) {
    ...
    if (video_state->picture_queue_size == 0) {
      ...
      // 取得当前帧的显示时间戳
      video_state->video_current_pts = video_picture->pts;
      // 取得系统时间，作为当前帧播放的时间基准
      ...
      // 检查主同步时钟源
      if (video_state->av_sync_type != AV_SYNC_VIDEO_MASTER) {
        // 根据主时钟来判断Video播放的快慢，以主时钟为基准时间
        ref_clock = get_master_clock(video_state);
        // 计算图像帧显示与主时钟的时间差
...
// 视频解码线程函数，主要功能是分配解码帧缓存和SDL显示缓存，
// 然后进入循环解码(从队列中取数据帧，解码，计算时钟，显示)，释放视频数据帧/数据包缓存
int video_thread(void* arg) {
  ...
  while (1) {
    ...
    if (packet->data == flush_pkt->data) {
      // 重新解码前需要重置解码器
      avcodec_flush_buffers(media_state->video_codec_ctx);
      continue;
    }
    // =============
    pts = 0;
...
int stream_component_open(MediaState* media_state, int stream_index) {
  ...
  switch (codec_ctx->codec_type) {
    case AVMEDIA_TYPE_AUDIO:
      ...
      //// 音频时钟与主同步源累计时差加权系数
      //media_state->audio_diff_avg_coef = exp(log(0.01 / AUDIO_DIFF_AVG_NB));
      //media_state->audio_diff_avg_count = 0;  //音频不同步计数初始化
      //// Correct audio only if larger error than this.
      //media_state->audio_diff_threshold = 2.0 * 1024 / codec_ctx->sample_rate;
    ...
    case AVMEDIA_TYPE_VIDEO:
      ...
      media_state->video_current_pts_time = av_gettime();
...
int decode_thread(void* arg) {
  ...
  while (1) {
    ...
    // ==============================
    // 检查[快进]/[快退]操作标志位是否开启
    if (media_state->seek_req) {
      int stream_index = -1;
      // 取得[快进]/[快退]操作后的参考时间戳
      int64_t seek_target = media_state->seek_pos;
      // the + -2 is due to rounding being not done in the correct direction in generation of the seek_pos/seek_rel variables
      int64_t seek_min = media_state->seek_rel > 0
                             ? seek_target - media_state->seek_rel + 2
                             : INT64_MIN;
      int64_t seek_max = media_state->seek_rel < 0
                             ? seek_target - media_state->seek_rel - 2
                             : INT64_MAX;

      if (media_state->video_index >= 0) {
        stream_index = media_state->video_index;
      } else if (media_state->audio_index >= 0) {
        stream_index = media_state->audio_index;
      }
      ...
      //      media_state->fmt_ctx->streams[stream_index]->time_base);
      //}
      // 根据[快进]/[快退]操作后的时间戳，跳到指定帧(该函数只能跳到离指定帧最近的关键帧)
      //if (av_seek_frame(media_state->fmt_ctx, stream_index, seek_target,
      //                  media_state->seek_flags) < 0) {
      //  std::cout << media_state->filename << ": error while seeking..."
      //            << std::endl;
      //}
      if (avformat_seek_file(media_state->fmt_ctx, -1, seek_min, seek_target,
                             seek_max, media_state->seek_flags) < 0) {
        std::cout << media_state->filename << ": error while seeking..."
                  << std::endl;
      } else {
        // 在执行[快进]/[快退]操作后，立刻清空缓存队列，并重置音视频解码器
        if (media_state->audio_index >= 0) {
          // 清除音频队列缓存，释放队列中所有动态分配的内存
          Packet_Queue_Flush(&media_state->audio_queue);
          // 将flush_pkt插入音频数据包队列，执行重置音频解码器操作avcodec_flush_buffers
          Packet_Queue_Put(&media_state->audio_queue, flush_pkt);
        }
        if (media_state->video_index >= 0) {
          Packet_Queue_Flush(&media_state->video_queue);
          Packet_Queue_Put(&media_state->video_queue, flush_pkt);
        }
      }
      media_state->seek_req = 0;
    }
    // ==============================
    // 检查音视频编码数据包队列长度是否溢出
...
// 设置[快进]/[快退]状态参数
void stream_seek(MediaState* media_state, int64_t pos, int64_t rel) {
  // 检查[快进]/[快退]操作标志位是否开启
  if (!media_state->seek_req) {
    media_state->seek_pos = pos;
    media_state->seek_rel = rel;
    // 确定[快进]还是[快退]操作
    media_state->seek_flags &= ~AVSEEK_FLAG_BYTE;
    media_state->seek_flags = rel < 0 ? AVSEEK_FLAG_BACKWARD : 0;
    media_state->seek_req = 1;
  }
}
...
int main(int argc, char* argv[]) {
  ...
  media_state->av_sync_type = DEFAULT_AV_SYNC_TYPE;
  ...
  while (media_state->quit != 1) {
    ...
    switch (event.type) {
      case SDL_KEYDOWN:
        ...
        if (global_media_state && increase != 0.0) {
          // 取得当前主同步源时间戳
          pos = get_master_clock(global_media_state);
          // 检查其参数是否是“不是一个数字”,是数字的话，返回0
          if (isnan(pos)) {
            pos = (double)global_media_state->seek_pos / AV_TIME_BASE;
          }
          // 根据键盘操作更新主同步源时间戳(AV_TIME_BASE为时间戳基准值)
          pos += increase;
          if (global_media_state->fmt_ctx->start_time != AV_NOPTS_VALUE &&
              pos < global_media_state->fmt_ctx->start_time /
                        (double)AV_TIME_BASE) {
            pos =
                global_media_state->fmt_ctx->start_time / (double)AV_TIME_BASE;
          }
          // 根据主同步源时间戳设置查找位置
          stream_seek(global_media_state, (int64_t)(pos * AV_TIME_BASE),
                      (int64_t)(increase * AV_TIME_BASE));
        }
        break;
      // 退出进程事件
      case FF_QUIT_EVENT:
