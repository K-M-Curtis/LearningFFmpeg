// FFmpeg5.1 的 resampling audio 的例子
// 将一段"双声道-48000-DBL"格式的音频转为"三声道-44100-S16"的音频文件
...
int main(int argc, char* argv[]) {
	...
	do {
		...
		if (dst_nb_samples > max_dst_nb_samples) {
			av_freep(&dst_data[0]);
			ret = av_samples_alloc(dst_data, &dst_linesize, dst_nb_channels, dst_nb_samples, dst_sample_fmt, 0);
			if (ret < 0) {
				break;
			}
		...
		fwrite(dst_data[0], 1, dst_bufsize, dst_fp);
	} while (t < 10);
